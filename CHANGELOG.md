...
- **`setup.sh` announces the deployed installer version**: the deployment summary now prints `Installed installer version: vX.Y.Z` (parsed from the deployed `install_coding_tools.sh` header via a new `get_installer_version` helper) and appends the version inline next to each installed script path. Previously the summary listed only paths, so it was not obvious which installer version had just been deployed.
...
- **Claude Code CLI installation failure on Windows**: The Anthropic `install.cmd` script could terminate or corrupt the parent installer session. Third-party batch installers are now executed in an isolated child `cmd.exe` via `:run_cmd_script_isolated`, preventing session contamination.
- **npm global list check syntax**: Removed extraneous parentheses wrapping the `for /f` loop in `:is_claude_in_npm` that caused parse errors on some Windows configurations.
...
- **ast-grep auto-installation**: After MoAI-ADK installation, the installer now automatically checks for ast-grep and installs it via npm if not present. ast-grep is required for MoAI-ADK security scanning features.
...
- **oh-my-opencode installed version precedence**: `install_coding_tools.sh` and `install_coding_tools.bat` now prioritize the active conda env's global npm-installed `oh-my-opencode` version over cache/spec fallbacks.
- **Batch parity**: Windows addon version detection now mirrors Unix behavior (npm global -> OpenCode cache -> pinned spec -> Unknown).
