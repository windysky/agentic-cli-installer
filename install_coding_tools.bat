...
REM - v1.13.0: Security + Windows parity. Gate insecure curl -k behind consent; block the Claude
REM            binary on Authenticode HashMismatch; add distinct "upgrade" action state (ACT=3,
...
REM Install or verify ast-grep (required for MoAI-ADK security scanning)
REM ast-grep provides AST-based code analysis for security and quality scans
:install_ast_grep
	echo.
	echo %CYAN%[MOAI DEPENDENCY]%NC% Checking ast-grep ^(AST-based code analysis^)...

	REM Check if ast-grep is already installed
	where ast-grep >nul 2>nul
	if not errorlevel 1 (
		for /f "delims=" %%v in ('ast-grep --version 2^>nul') do (
			if not "%%v"=="" echo   %GREEN%[OK]%NC% ast-grep already installed ^(%%v^)
		)
		exit /b 0
	)

	REM Check for npm (required for installation)
	if not defined CONDA_NPM call :resolve_conda_npm
	if errorlevel 1 (
		echo   %YELLOW%[WARNING]%NC% npm not found, cannot install ast-grep
		echo   %YELLOW%[WARNING]%NC% Without ast-grep, MoAI-ADK security scanning will be disabled
		echo   %YELLOW%Please install manually: npm install -g @ast-grep/cli%NC
		exit /b 0
	)

	REM Install ast-grep via npm
	echo   %BLUE%[INFO]%NC% Installing ast-grep via npm...
	echo   %BLUE%[INFO]%NC% ^(ast-grep is required for MoAI-ADK security scanning^)
	call "!CONDA_NPM!" install -g @ast-grep/cli >nul 2>nul
	if errorlevel 1 (
		echo   %YELLOW%[WARNING]%NC% Failed to install ast-grep via npm
		echo   %YELLOW%[WARNING]%NC% Without ast-grep, MoAI-ADK security scanning will be disabled
		echo   %YELLOW%Please install manually: npm install -g @ast-grep/cli%NC
		exit /b 0
	)
	...
	if not errorlevel 1 (
		for /f "delims=" %%v in ('ast-grep --version 2^>nul') do (
			echo   %GREEN%[SUCCESS]%NC% ast-grep installed ^(%%v^)
		)
	) else (
		echo   %YELLOW%[WARNING]%NC% ast-grep installed but command not found in PATH
